...
def assign_unique_id_on_profile_creation(sender, instance, created, **kwargs):
    if created and instance.country and not instance.unique_id:
        unique_id = generate_unique_id(instance.country)  # Pass full country name
        
        # Ensure uniqueness
        while Profile.objects.filter(unique_id=unique_id).exists():
            unique_id = generate_unique_id(instance.country)
        
        # Use update to avoid recursion
        Profile.objects.filter(pk=instance.pk).update(unique_id=unique_id)
